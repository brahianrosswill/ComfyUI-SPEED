...
def dct2_partial(x, h_keep, w_keep):
    """Compute only the top-left (h_keep × w_keep) block of dct2(x).

    Saves ≈ (1 - (h_keep·w_keep)/(H·W)) of the matmul work vs computing the full
    DCT then slicing. The win is meaningful for the initial downscale where
    scale ≤ 0.75 typically discards >40% of bins.
    """
    B, C, H, W = x.shape
    device, dtype = x.device, x.dtype
    D_h = _dct_matrix(H, device=device, dtype=dtype)[:h_keep]   # [h_keep, H]
    D_w = _dct_matrix(W, device=device, dtype=dtype)[:w_keep]   # [w_keep, W]
    x = x.reshape(B * C, H, W)
    x = D_h @ x          # [B*C, h_keep, W]
    x = x @ D_w.T        # [B*C, h_keep, w_keep]
...
def _cos_taper_1d(n, kept, taper, device=None, dtype=None):
    """Return weights w[0:n]: 1.0 inside [0, kept - taper], smooth cosine ramp
    down to 0.0 over [kept - taper, kept], then 0.0 for [kept, n).

    `taper=0` reproduces the hard-truncation behaviour. Increasing it widens
    the crossfade ring (reduces ringing at the cost of slightly more low-pass)."""
    w = torch.ones(n, device=device, dtype=dtype)
    taper = max(0, min(taper, kept))
    if taper > 0:
        ramp_start = kept - taper
        idx = torch.arange(taper, device=device, dtype=dtype)
        # Hann-style half-cosine: 1 at start, 0 at end.
...
def _preserve_mask_2d(h_hi, w_hi, h_lo, w_lo, taper, device, dtype):
    """2D outer-product taper: rectangle of 1s in low-freq core, smooth ramp
    to 0 inside [h_lo-taper, h_lo] × [w_lo-taper, w_lo], 0 outside."""
...
def spectral_expand(x_lo, sigma_i, scale_lo, scale_hi, target_h, target_w,
                    taper=8):
    """Grow latent from scale_lo to scale_hi via zero-padded DCT + noise fill in
    the new high-frequency bins. Returns (x_new, sigma_aligned).

    `taper` controls how many DCT-bin rows/cols are crossfaded at the
    preserved↔noise boundary. 0 = original hard truncation behaviour."""
    ...
    # Exact math directly on sigma (no shift required).
    ...
    xi_new = xi_padded * preserved_w + sigma_val * noise * noise_w
...
def initial_downscale(x, scale, taper=8):
    """Truncate x's DCT to the top-left `scale` fraction with a cosine taper at
    the edge. `taper=0` = original brick-wall truncation.

    Uses the partial DCT to avoid computing-and-discarding the high-frequency
    bins. Taper is applied only to the kept block's last `taper` rows/cols.
    """
    ...
    # Only compute the kept block of the DCT.
    xi_kept = dct2_partial(x_4d.float(), h0, w0)
    if taper > 0:
        # The taper only affects bins in the last `taper` rows/cols of the kept
        # block — outside those, the window is 1.0.
...
# We call comfy's stock k_diffusion samplers between SPEED transitions. This way
# we don't have to reimplement each solver — every sampler comfy ships works,
# and any new ones added to comfy/k_diffusion light up for free.
#
# Multistep solvers (dpmpp_2m, res_multistep, er_sde, lms, …) keep state inside
# the call. When SPEED triggers a spectral expansion we end the current segment
# and start the next one fresh — multistep state resets to first-order at the
# segment boundary. That's the same gracefully-degrade behaviour we had with
# the explicit step functions; cleaner here because comfy's own sampler handles
# its own state.


def _get_comfy_sampler_fn(name):
    """Look up a sample_<name> function in comfy.k_diffusion.sampling. Raises
    if the requested sampler isn't available in this comfy install."""
...
def _list_available_samplers():
    """Return the sorted list of comfy-installed sample_<name> functions that
    fit SPEED's call contract (take model, x, sigmas as first three args).

    Returns a small built-in fallback when comfy isn't importable (unit tests)."""
    try:
        import comfy.k_diffusion.sampling as kds
        ...
        return ["euler", "euler_ancestral", "heun", "dpmpp_2m", "er_sde",
                "res_multistep"]
    # Exclusion list:
    #   dpm_fast / dpm_adaptive — use their own sigma scheduling internally and
    #   ignore the supplied sigmas, so they can't be sliced into segments.
    #   sample_lcm — distillation-specific; usually 2–8 steps total, not enough
    #   room for SPEED transitions.
...
def _segment_callback(outer_cb, segment_start_idx):
    """Wrap comfy's per-step callback so its (segment-local) `i` is translated
    back to the global step index. Lets the UI progress bar still make sense."""
    if outer_cb is None:
        return None
...
def sample_speed(model, x, sigmas, extra_args=None, callback=None, disable=None,
    ...
    boundary_idxs = []          # index into sigmas[] where transition fires
    boundary_scales = []        # (old_scale, new_scale) for each boundary
    transition_idx = 0
    ...
    sigmas = sigmas.clone()  # we may mutate to re-space after expansion
    ...
    for seg_i, seg_start in enumerate(segment_starts):
        ...
        if seg_i < len(boundary_idxs):
            ...
            x, sigma_aligned = spectral_expand(
                ...
                H_full, W_full, taper=taper,
            )
            current_scale = new_scale
            # Re-space remaining sigmas so the next segment walks σ_aligned → 0
            # (paper §4.3).
